...
def api_report():
    ...
    prompt = f"""你是资深皮肤科AI辅助诊断专家。请根据以下分类结果生成一份Markdown格式的完整医疗报告。
...
## 输出要求
请直接输出以下Markdown格式的医疗报告（不要输出代码块标记，纯Markdown正文），每节2-3句话：
...
*⚠️ 本报告由AI生成，仅供参考，不能替代专业医疗诊断。如有不适请及时就医。*"""
    ...
    messages = [
        {"role": "system", "content": "你是一位资深皮肤科AI辅助诊断专家。请严格按照用户要求的Markdown格式输出，使用中文，每节简洁专业。"},
