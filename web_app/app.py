...
def api_report():
    ...
    prompt = f"""你是一位资深皮肤科AI辅助诊断专家。请根据以下分类结果，生成一份结构完整的中文医疗报告。
...
症状: {kb.get('symptoms','')}
治疗: {kb.get('treatment','')}
注意事项: {kb.get('precautions','')}

## 报告格式要求
请严格按照以下格式生成报告，每项2-3句话，简洁专业：

📋 疾病概述
（简述该疾病的定义、发病机制、流行病学特点）

🔍 临床特征
（描述典型皮损形态、好发部位、伴随症状）

🩺 鉴别诊断
（列出2-3个需要区分的相似疾病，简要说明鉴别要点）

⚠️ 风险评估
（根据风险等级给出评估意见，HIGH需强调紧急就医）

💊 治疗建议
（分阶梯列出治疗选项：一线治疗→二线治疗→辅助治疗）

📅 随访计划
（建议复诊频率、需要警惕的危险信号）

🚑 就医指引
（建议就诊科室、需携带的资料、紧急情况处理）

---

⚠️ 本报告由AI生成，仅供参考，不能替代专业医疗诊断。如有不适请及时就医。"""

    messages = [
        {"role": "system", "content": "你是一位资深皮肤科AI辅助诊断专家，擅长撰写结构清晰、专业准确的医疗报告。请严格按照用户要求的格式输出，每项2-3句话，使用中文。"},
