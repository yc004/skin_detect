...
def segment_lesion(image_bgr: np.ndarray) -> tuple:
    """
    Segment the skin lesion from background.
    ...
    # Try Otsu on L-channel
    _, thresh_l = cv2.threshold(L, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)

    # Also try a multi-channel approach: threshold the A channel (redness)
    ...
    # Use largest contour (assumes single lesion per image)
    largest = max(contours, key=cv2.contourArea)
    area = cv2.contourArea(largest)

    # If too small (<5% of image), fall back to near-full-image
    if area < 0.05 * h * w:
        return cleaned, None
    ...
    # Normalize to YOLO format: class_id cx cy w h (all 0-1)
    cx = (x + bw / 2) / w
    cy = (y + bh / 2) / h
    nw = bw / w
    nh = bh / h
