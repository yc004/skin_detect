...
\begin{titlepage}
    ...
    {\zihao{-4}\heiti 作品名称：} & {\zihao{-4}\heiti 基于改进 ConvNeXt 的多类别} \\[6pt]
    & {\zihao{-4}\heiti 皮肤疾病智能分类系统} \\[12pt]
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{img/model_baseline.png}
    \caption{基线模型架构（ConvNeXt V1 + AdaptiveAvgPool2d）}
    \label{fig:architecture_baseline}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{img/model_improved.png}
    \caption{改进模型架构（ConvNeXt V2 + GeM Pooling + EMA）。改进点：① 每个 block 新增 GRN 层；② AdaptiveAvgPool2d → GeM Pooling；③ 训练阶段引入 EMA 权重平滑。}
    \label{fig:architecture_improved}
\end{figure}
