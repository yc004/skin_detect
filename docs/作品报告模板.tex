...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{img/architecture_comparison.png}
    \caption{基线模型（ConvNeXt V1 + AvgPool）与改进模型（ConvNeXt V2 + GeM + EMA）架构对比。改进模型在三个位置做出了改动：① V1→V2，每个 block 新增 GRN 层；② AdaptiveAvgPool2d→GeM Pooling；③ 训练阶段引入 EMA 权重平滑。}
